...
class ExcelToolbyXlsxWriter:
    ...
    def __init__(self,path,book_name_xls,sheet_name_xls) :
        super(ExcelToolbyXlsxWriter).__init__()
        self.path = path
    ...
    def write_xls(self, value):
        ...
        workbook = xlsxwriter.Workbook(self.excel_loc)  # 新建一个工作簿
        sheet = workbook.add_worksheet(self.sheet_name_xls)  # 在工作簿中新建一个表格
    ...
    def write_xls_append(self, value):
        index = len(value)  # 获取需要写入数据的行数
        workbook = xlsxwriter.open_workbook(self.excel_loc)  # 打开工作簿
...
def test():
    ET = ExcelToolbyXlsxWriter("excel/",'xls格式测试工作簿.xls','xls格式测试表')
    value_title = [["姓名", "性别", "年龄", "城市", "职业"], ]
