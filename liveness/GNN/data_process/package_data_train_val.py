...
class PackageData_train_val(torch.utils.data.Dataset):
    def __init__(self, data_loc, readorwrite, filename = None):
        ...
        if readorwrite == "read":
            try:
                with open(os.path.join(data_loc , "DataSet4_train_and_validate.pkl"), "rb+") as f:
                    # f = pickle.load(f)
                    print("111111111")
                    f1 = pickle.load(f)
                    f.close()
            ...
            x_train, x_validate, y_train, y_validate = train_test_split(x_data, y_data, test_size=0.1, random_state=7)
        ...
        print('train, validate:', len(self.train), len(self.validate))
...
def write_file(path, name):
    path = data_root
    train_file_name = name + '_train_and_validate.pkl'

    dataset = PackageData_train_val(data_root, "write")

    start = time.time()
    with open(os.path.join(path, train_file_name) ,'wb') as f:
        pickle.dump([dataset.train,dataset.validate],f)
...
if __name__ == '__main__':
    save_root = '/home/qhd/code/liveness/data_generation/UPN/DS4/UPN_RG_TV/'
    data_root = '/home/qhd/code/liveness/data_generation/UPN/DS4/UPN_RG_TV/'
